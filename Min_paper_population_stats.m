...
for subjectNum=1:numSubject
    ...
    for sessionNum=1:numel(sessionDays)
        ...
        for cellNum=1:numCells % stats start here
            ...
            %             responses=mean(allTraces(:,preAlignWindow/interval-3:preAlignWindow/interval+2),2)'; %response asverage over 6 frames
            %                         stats(subjectNum).taskRelated(sessionNum).pVals(cellNum,1)=signrank(baselines,responses); %or paired ttest
            ...
            if stats(subjectNum).taskRelated(sessionNum).indices(cellNum)
                ...
                catCriterion=stats(subjectNum).taskRelated(sessionNum).peakTime(cellNum); %sigDiffTimes(1);
...
%                                 box off;
            ...
            %% now compare trials by outcome (success, fail)
